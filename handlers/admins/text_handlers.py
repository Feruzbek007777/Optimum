...
from database.database import get_approved_students, add_announcement, get_courses, get_all_teachers, delete_teacher, \
    delete_course, get_all_admin_groups
from keyboards.default import admin_menu_keyboard, yes_no_keyboard
from keyboards.inline import generate_courses_keyboard, generate_teachers_keyboard, generate_courses_delete_keyboard, \
    generate_teachers_delete_keyboard, generate_groups_keyboard
...
def setup_admin_text_handlers(bot) :
    ...
    @bot.message_handler(
        func=lambda message : message.text == "ℹ️ Kursga ma'lumot qo'shish" and message.from_user.id in ADMINS)
    ...
    @bot.message_handler(
        func=lambda message : message.text == "🗑️ Ustozni o'chirish" and message.from_user.id in ADMINS)
    ...
    @bot.message_handler(
        func=lambda message : message.text == "👥 Guruhlarga xabar yuborish" and message.from_user.id in ADMINS)
...
def process_announcement(message, bot) :
    ...
    bot.send_message(message.chat.id, "✅ E'lon saqlandi!", reply_markup=admin_menu_keyboard())